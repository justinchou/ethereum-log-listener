...
class BlockHandler extends EventEmitter {
  ...
  subscribeBlock(withBlockInfo) {
    ...
    .on("data", blockHeader => {
      ...
      if (withBlockInfo) this.getBlockInfo(blockNumber);
  ...
  pullBlockNumber() {
    let interval = setInterval(() => {
      ...
      this.web3.eth.getBlockNumber()
        .then(blockNumber => {
          clearTimeout(delayTicker);
          console.log("Pong Block %sms %s", (Date.now() - startTime), blockNumber);
          this.timesFailedConnections = 0;

        })
        .catch(err => {
          clearTimeout(delayTicker);
          console.log("Pong Failed %sms", (Date.now() - startTime), err);
          this.timesFailedConnections += 1;
        });
...
BlockHandler.parseBlockInfo = parseBlockInfo;
