...
function initOriginContractAddress(cacheHandler, addresses) {
  addresses.forEach(async address => {
    try {
      const savedContract = await cacheHandler.writeContractAddress(address, 'origin');
      logger.debug('Save Init Contract Address [ %s ] [ %j ]', address, savedContract);
    } catch (err) {
      logger.error('Save Init Contract Address Failed [ %s ]', err.message);
    }
  });
}
exports.initOriginContractAddress = initOriginContractAddress;
