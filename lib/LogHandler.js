...
class LogsHandler extends EventEmitter {
  ...
  async _filterLog(log) {
    ...
    if (!contractName && !eventName) return;
    ...
    if (log.removed) {
      await this._logRevert.call(this, blockNumber, contractName, log);
    } else {
      await this._logRecvier.call(this, blockNumber, contractName, log);
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(logs => {
        ...
        if (Array.isArray(logs) && logs.length > 0)
          logs.forEach(log => this._filterLog(log));
