...
class LogsHandler extends EventEmitter {
  constructor (config, web3, contracts, topic2name, topic2event, {
  ...
  async _parseLog(contractName, logItem) {
    return new Promise((resolve, reject) => {
      ...
      resolve({parsedLog: log, origin: logItem});
    });
  }

  async _logRecvier(blockNumber, contractName, log) {
    const {parsedLog, origin} = await this._parseLog(contractName, log);
    ...
    this.emit('log', blockNumber, contractName, parsedLog, origin);
    return true;
  }
  ...
  async _logRevert(blockNumber, contractName, log) {
    ...
    this.emit('revert', blockNumber, contractName, parsedLog, origin);
    return true;
  }

  async _filterLog(log) {
    let blockNumber  = log.blockNumber;
    let contractName = this.topic2name[log.topics[0]];
