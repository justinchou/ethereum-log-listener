...
class CacheHandler extends EventEmitter {
  ...
  // 获取当前未处理的 Block 列表 - @TODO BUG: 最小块左面丢失块
  ...
  async removeRedisBlockNumber(blockNumber) {
    let delAmount = await this._redis.delAsync(`s_blk:${blockNumber}`);
    logger.debug("BlockNumber Into Redis [ %s ] => ", blockNumber, delAmount)

    if (delAmount === 1) return true;
    return false;
  }
